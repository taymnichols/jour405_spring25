...
Setting aside other factors, we have to first ask if home values have increased - basically fact checking their statements.
...
t-test is either testing one thing or two things - here we are testing one thing: transit line's impact on home values
t.test takes all sales prices and compare them to prior mean - alternative = greater means that the home values are greater than the mean

t-statistic 
df = degrees of freedom = number of samples in df minus 1 - how complicated things could get, how many possible variations. the lower df seems good but is bad, we want more data.

p value = probability that what we are seeing is random chance - if it's low, we can say it's not random and we can reject the null hypothesis. Low is 0.05, but there are some problems with that.

95 percent confidence interval - high degree of confidence the true average / mean is is somehwere around here (low end of true mean) - basically tells us the mean is actually probably higher than what we thought previously
```{r}
...
alpha <- 0.05 #low p-value threshold
...
Local officials say the new transit line has driven property values up in the area. An independent analysis of sales prices for 14 homes show that average home values have increased by roughly 7 percent since construction of the new line, an increase of about $19,500. While it's true that increased access to public transit could be driving demand, a variety of other factors could also be impacting home sales in the neighborhood.
