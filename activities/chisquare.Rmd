...
Yes, the observed frequencies don't match up perfectly. There are 15 less 5-star reviews than expected, 8 more 4 star reviews, 7 less 3-star reviews, 12 more 2-star reviews and 2 more 1-star reviews. All in all, it seems like people were less likely to rate things 5 stars than expected. Things were more heavily tipped towards 4-star or 2-star reviews than expected. I think 2-star reviews took up the most of those reallocated ratings.

### Task 2: Chi-Square Test
To perform the chi-square test, we will use the chisq.test() function from the vcd package:

```{r}
chisq_result <- chisq.test(movie_ratings$Observed.Frequency, p = movie_ratings$Percentage) #expected probability values - uses this to determine distribution
chisq_result
```
...
```{r}
alpha <- 0.05 #sets the p-value I believe
...
Since the chi-square statistic is greater than the critical value, we can reject the null hypothesis - our results ARE significantly different from what we expected, AND it is statistically significant because the p-value is below .05. Basically this year's movies weren't as good as last year's movies.
